...
import { Info, Code2, Megaphone, Mail } from 'lucide-react';
...
export default function AppHeader({ builderMode, onBuilderModeChange }: AppHeaderProps) {
  ...
  return (
    <header className="border-b border-border bg-card shadow-sm">
      <div className="flex items-center justify-between px-5 py-2.5">
        ...
        {/* Right: mode toggle (marketing / transactional) */}
        <div className="flex items-center gap-1 rounded-lg border border-border overflow-hidden bg-muted p-0.5 flex-shrink-0">
          <button
            type="button"
            onClick={() => onBuilderModeChange('marketing')}
          ...
          <button
            type="button"
            onClick={() => onBuilderModeChange('transactional')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-medium transition-all ${
      ...
      {/* Platform sub-tabs — only show for marketing mode */}
      {builderMode === 'marketing' && (
